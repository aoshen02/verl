...
def _merger(state_dict):
    ...
    merger.config = SimpleNamespace(
        operation="merge", lora_only=True, hf_upload=False
    )
...
def test_lora_only_export_skips_full_model_save(tmp_path):
    ...
    merger.save_hf_model_and_tokenizer = lambda state: pytest.fail(
        "full model save must not run"
    )
...
def test_lora_only_export_rejects_base_weights():
    merger = _merger(
        {
            "base_model.model.layers.0.q_proj.lora_A.default.weight": torch.ones(
                2, 4
            ),
